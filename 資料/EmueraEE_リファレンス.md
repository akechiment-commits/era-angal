...
`HTML_PRINT` で `<img>` を出すと、後続のテキストが画像の右側に回り込む。`<br>` を複数追加して次のテキストを画像の下に追いやる必要がある。画像高さ 150px なら `<br>` × 8〜12 個、400px なら 20〜25 個が目安。

### ⚠️ HTMLレンダラー対応タグ／属性（厳守）

Emuera の HTML レンダラーは**極めて制限された独自パーサ**。一般的な HTML/CSS はほとんど通らない。実際にエラーを踏んで確定した仕様:

**対応するタグ:**
- `<img src='...' height='NNpx'>` — **画像表示のみ用途**
- `<br>` — 改行

**非対応で実行時エラーになるもの（`<img>タグの属性名XXは解釈できません`）:**
- `align='right'` / `align='left'`
- `style='...'`（CSS 属性全般）
- `hspace` / `vspace` も未検証（使うな）

**非対応タグ（`html文字列"..."のタグ解析中にエラー`）:**
- `<table>` / `<tr>` / `<td>`
- `<div>` / `<span>`（未検証だが信用するな）

**ERB 書式文字列の `%` 罠:**
- `@"..."` 内に `%` があると ERB が変数展開 `%VARNAME%` として解釈してパース失敗
- `width='100%'` のような記法は書けない。ピクセル固定にするしかない

**結論: 画像の位置調整は不可能。左表示のまま `<br>` / `PRINTL` で回り込みを制御するのが唯一の選択肢。**
