...
- **調査の前に必ずリポジトリ内のデータを確認する**
  ...
  - ⚠`tools/prompt_*.txt`（約120件サンプル＋自動生成プロフィール）と `tools/output/*_分析.txt`・`*_関係性分析.txt`（AI派生分析）は**誤誘導の元だったため2026-06-06に全削除済み**。自動プロフィールの一人称等が不正確だった（例: ゆゆは「一人称: うち」と誤記載だが、全件精査では自称「ゆゆ」）。**復活させず、原作判断は必ず上記の全件(`output/<名>.txt`)＋原典(`scenarios/`)で行う**
