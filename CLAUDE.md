...
- **調査の前に必ずリポジトリ内のデータを確認する**
  ...
  - ★**全件ファイルだけで文脈が分からない時は、必ず該当シナリオを開く**：`output/<名>.txt` の各行頭 `[番号]` ＝シナリオ番号。掛け合いの相手・状況・「あのひと」等の指示語が単独セリフでは読めない場合、対応する `tools/scenarios/<番号>.txt` を開いて他キャラ込みの全文脈を確認してから判断する（例: くるみ`[2428]`→`scenarios/2428.txt`＝こよいとの試食会で木登りした場面）
  - キャラ間の関係性: `tools/output/<キャラ名>_関係性分析.txt`（共演話数・他キャラとの掛け合い情報を含む。単独の全件ファイルでは拾えない関係性の根拠取りに使う／AI派生なので重要点は `scenarios/` で裏取り）
  - ⚠`tools/prompt_*.txt`（約120件サンプル＋自動生成プロフィール）と `tools/output/*_分析.txt`（単独AI分析）は**誤誘導の元だったため2026-06-06に削除済み**。自動プロフィールの一人称等が不正確だった（例: ゆゆは「一人称: うち」と誤記載だが、全件精査では自称「ゆゆ」）。**復活させず、原作判断は必ず全件(`output/<名>.txt`)＋原典(`scenarios/`)で行う**（関係性は上記 `_関係性分析.txt` を併用）
